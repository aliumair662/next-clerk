...
const EventList = () => {
    ...
    return (<>
        <div className="flex flex-wrap items-end justify-between gap-4">
            ...
            <Button onClick={retrieveData} className="cursor-pointer">Fetch Events</Button>
        </div>
        <ul className="mt-10">
            {error ? <li>{error}</li> : events.map((event, index) => (
                <>
                    <li key={event.id}>
            ...
            {!events.length && <li>{`Please click "Fetch Events" button to display data.`}</li>}
        </ul>
    </>);
}
